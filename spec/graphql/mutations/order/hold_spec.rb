...
describe Mutations::Order::Hold, type: :mutation do

  subject { mutation build_query, context: {current_user: contractor.user} }

  let(:mutation_type) { :orderHold }
  ...
  let(:order1) {
    create(:order, quantity: 20, total_price: 505, latitude: 0, longitude: 0,
  ...
  describe "when order is valid" do
    it "calls operation and returns success" do
      expect(Operations::Order::Hold).to receive(:call).with(
